...
The public API lets you drive your KK WABA instance from your own
...
```
Authorization: Bearer kkwaba_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** KK WABA
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer kkwaba_live_xxx"
```
...
Rather than polling, register an endpoint and KK WABA will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; KK WABA keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer kkwaba_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/kk-waba", "events": ["message.received"] }'
...
Headers: `X-KK-WABA-Event`, `X-KK-WABA-Webhook-Id`, and `X-KK-WABA-Signature`.

### Verifying the signature

`X-KK-WABA-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages KK WABA stores (inbox + API sends), not broadcast-only
