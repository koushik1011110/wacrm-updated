...
export async function POST(request: Request) {
  try {
    ...
    // KK WABA is single-tenant-per-WhatsApp-number — letting two accounts
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one KK WABA user.',
        },
        { status: 409 }
      )
    }
